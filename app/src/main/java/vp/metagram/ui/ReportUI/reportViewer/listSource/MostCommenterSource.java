...
public class MostCommenterSource extends ReportListSource
{
    long FIPK = -1;
    String searchValue;
    int lastID = -1;

    String QueryFromPart =  "Select Others.IPK, CommentNo, Others.Username,Others.PictureURL from Engagement Left Join Others\n" +
            " On EngageIPK = Others.IPK\n" +
            "  Where FIPK = %d and StatJobID = %d and CommentNo > 0 and Others.Username like '%%%s%%' Order By CommentNo Desc ";
    ...
    public MostCommenterSource(long FIPK,  List<ReportUser> sourceList, String searchValue)
    {
        super(sourceList);
        this.FIPK = FIPK;
        ...
        else
        {
            ...
            if (result.moveToFirst())
            {
                while ( !result.isAfterLast() )
                {
                    ...
                    reportUser.commentNo = result.getInt(result.getColumnIndex("CommentNo"));
