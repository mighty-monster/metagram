...
public class addedLikesAdaptor extends ChartAdaptor
{
    long IPK;
    List<Pair<Long,Pair<Long,Long>>> results = new ArrayList<>();

    public addedLikesAdaptor(Context context, ChartType chartType, QueryStepType stepType, long IPK) throws Exception
    {
    ...
    public void fillResult() throws Exception
    {
        ...
        if ( queryResult.moveToFirst() )
        {
            ...
            while ( !queryResult.isAfterLast() )
            {
                ...
                try
                {
                    tubeLikes = resultObject.getLong(objectName);
                }
                catch (Exception e)
                {
                    e.printStackTrace();
                ...
                {
                    noOfAddedLikes = tubeLikes - noOfLikes;
                    noOfLikes = tubeLikes;

                    Pair<Long,Pair<Long,Long>> newItem = new Pair(
                            new Long(index),
                            new Pair<>(new Long(endTime),noOfAddedLikes));
                    results.add(newItem);
                    index++;
                }
