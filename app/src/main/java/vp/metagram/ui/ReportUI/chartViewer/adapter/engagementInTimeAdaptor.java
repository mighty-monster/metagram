...
public class engagementInTimeAdaptor extends ChartAdaptor
{
    long IPK;
    List<Pair<Long,Pair<Long,Float>>> results = new ArrayList<>();

    public engagementInTimeAdaptor(Context context, ChartType chartType, QueryStepType stepType, long IPK) throws Exception
    {
    ...
    public List getEntries() throws Exception
    {
        ...
        if (chartType == barChart)
        {
            ...
            for (Pair<Long,Pair<Long,Float>> pair:results)
            {
    ...
    public void fillResult() throws Exception
    {
        ...
        {
            long index = 0;
            String objectName = "rankEngagement";
            while ( !queryResult.isAfterLast() )
            {
                ...
                try
                {
                    YValue = (float) resultObject.getDouble(objectName) * 100;
                }
                catch (Exception e)
                {
                    e.printStackTrace();
                ...
                Pair<Long,Pair<Long,Float>> newItem = new Pair(
                        new Long(index),
                        new Pair<>(
                                new Long(endTime),
                                YValue));
    ...
    public IAxisValueFormatter getXAxisValueFormatter() throws Exception
    {
        return (value, axis) ->
        {
            ...
            {
                Pair<Long, Pair<Long, Float>> pair = results.get(index);
                if ( pair != null )
                {
