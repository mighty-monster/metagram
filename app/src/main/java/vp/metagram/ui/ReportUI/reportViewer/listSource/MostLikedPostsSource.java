...
public class MostLikedPostsSource extends ReportListSource
{
    long FIPK = -1;
    String searchValue;
    ...
    public MostLikedPostsSource(long FIPK, List<ReportMedia> sourceList, String searchValue)
    {
        super(sourceList);
        this.FIPK = FIPK;
    ...
    public List<ReportMedia> getNextList() throws Exception
    {
        ...
        else
        {
            ...
            String sqlText = String.format(Locale.ENGLISH,"Select MPK, MiniLink, MID, PostType, LikeNo, CommentNo,ViewNo,PicUrl,Caption,URLs from Posts  Left Join Posts_Info \n" +
                    "    on Posts_Info.FMPK = Posts.MPK and Posts_Info.StatJobID = %d" +
                    "     Where FIPK = %d and Posts_Info.CommentNo Is Not NULL and Posts.Caption like '%%%s%%' Order By LikeNo desc limit %d OFFSET %d\n" +
            ...
            if (result.moveToFirst())
            {
                while ( !result.isAfterLast() )
                {
                    ...
                    reportMedia.extraInfo = result.getInt(result.getColumnIndex("LikeNo"));
